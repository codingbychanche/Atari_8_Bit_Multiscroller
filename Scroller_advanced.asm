; Scroller advanced
;
; Improved version of "Scroller.asm"
;
...
;
; Cursor is not frozen in center of playfield. When reaching the borders
; of the playfield, cursor is moved towards the borders allowing it to 
; reach the whole playfield.
;
; Plot routine now can also read char from screen ram (set 'readflag' to a val > 0).
;
; "Disadvantage": Screen always moves unless cursor is near either of the playfeld boders.
;
...
windowx		equ	40	; x- size of screen
windowy		equ 18	; y- size of screen		
...
movePFUp
	...
	jmp out
dd
	lda ycount			; Playfield scrolled all the way down?
	cmp #pfheight	
	bne ok1				; No: Scroll up!
	jsr move cursorDown ; Pf scrolled all the way down, just move cursor down.
	jmp out				; Leave.
ok1
...
movePFDown			
	lda localy
	cmp centery			; Same principle as above.
	beq uu				; Check if cursor is centered, if not, move cursor.
	jsr moveCursorUp	; If cursor is centered, scroll playfield
uu
	lda ycount		
	bne ok2
	jsr moveCursorUp
	jmp out
ok2
	lda vclocks			; Also the same as above: Check if finescroll is done
	beq hardDown		; If so, do hardscroll and then leave the vbi- Interrupt.
	dec vclocks
	lda vclocks
	sta $d405
	jmp out
